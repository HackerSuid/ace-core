...
void HtmRegion::AllocateColumns(
    float rfsz,
    float localActivity,
...
// overlap.
void HtmRegion::InitializeProximalDendrites()
{
    Column ***columns = (Column ***)input;

    for (unsigned int i=0, n=0; i<width; i++)
        for (unsigned int j=0; j<height; j++, n++)
...
void HtmRegion::RefreshLowerSynapses()
{
    Column ***columns = (Column ***)input;
...
void HtmRegion::CLA(bool Learning, bool allowBoosting)
{
...
void HtmRegion::SpatialPooler(bool Learning, bool allowBoosting)
{
    Column ***columns = (Column ***)input;
...
void HtmRegion::SequenceMemory(bool Learning, bool firstPattern)
{
    Column ***columns = (Column ***)input;
    /*
...
//                printf("Col [%d,%d] active\n", i, j);
                ...
                /*
                 * Search for cells that were predicted (depolarize) and
...
//                    printf("\tno cells predicted; activating all cells\n");
                ...
                if (!learnCellChosen) {
                     ...
                     */
                    _EnqueueSegmentUpdate(BestCell, segment, false);
                }
                /*
                 * After the learning cell is chosen, all other cells in the
                 * columns are set as non-learning. Unless this was the first
                 * pattern in a sequence, the last element of learningCells
                 * will be the learning cell for this column.
                 *
                if (!firstPattern) */
                for (int x=0; x<nc; x++)
        ...
        for (unsigned int j=0; j<height; j++) {
            ...
            for (int k=0; k<nc; k++) {
                ...
                for (int s=0; s<numSegs; s++) {
                     ...
                     */
                    if (segments[s]->IsActive() and !predflag) {
//                        printf("\tcell [%d,%d,%d] is predicting\n", i, j, k);
...
//                        printf("\n");
                        ...
                        predflag = true;
                        _EnqueueSegmentUpdate(cells[k], segments[s], true);
                    }
                }
                if (!predflag)
 ...
 */
void HtmRegion::_EnqueueSegmentUpdate(
    Cell *cell,
    DendriteSegment *segment,
    bool connected)
{
    SegmentUpdate *newSegment = new SegmentUpdate(cell, segment, connected);

    segmentUpdateList.push_back(newSegment);
}

void HtmRegion::_DequeueSegmentUpdate(
    ...
    bool reinforce)
{
    ...
    /*
     * One segment can learn to recognize multiple different patterns.
...
//        printf("\tcreating new learning segment\n");
    ...
    if (segment) {
//        printf("\treinforcing segment 0x%08x\n", segment);
...
int HtmRegion::LastActiveColumns()
{
    return numActiveColumns[1];
}

int HtmRegion::CurrentActiveColumns()
{
 ...
 */
bool HtmRegion::_EligibleToFire(Column *col)
{
 ...
 */
void HtmRegion::_ComputeInhibitionRadius(Column ***columns)
{
...
Column*** HtmRegion::GetInput()
{
    return (Column ***)input;
}

void HtmRegion::setlower(GenericRegion *reg)
{
    lower = reg;
}

void HtmRegion::sethigher(GenericRegion *reg)
{
    higher = reg;
}
